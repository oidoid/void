...
bench_test := \
  ...
  taskset --cpu-list 3 \
	go test --bench=. --count=15 --cpu=1 --p=1 --parallel=1 --run='^$$' $(go_tags) ./src/internal/demo/...
go_test_filter = \
...
packatlas_demo = go run ./src/cmd/packatlas --name=atlas --img-out=dist/demo/ --atlas-out=src/internal/demo/assets/atlas_bin.go --tags-out=src/internal/demo/tags/tags.go --tileset-manifest-out=$(tileset_manifest_demo) $(1) src/internal/demo/assets/atlas/
packboards_demo = go run ./src/cmd/packboards --tileset-manifest=$(tileset_manifest_demo) --out=src/internal/demo/boards $(1) src/internal/demo/assets/boards/
favicon_demo = \
	mkdir -p dist/demo/favicon; \
	for scale in 1 2 3 4 12 32; do \
		favicon=dist/demo/favicon/favicon$$((scale * 16)); \
		aseprite src/internal/demo/web/assets/favicon.aseprite --batch --color-mode=indexed --scale=$$scale --save-as=$$favicon.png; \
...
build-go:
	# no concurrency.
	GOOS=wasip1 GOARCH=wasm tinygo build $(tinygo_flags) -o $(out_demo) ./src/internal/demo/web/
...
clean:; rm --force --recursive dist/ src/internal/demo/assets/atlas_bin.go src/internal/demo/tags/tags.go
