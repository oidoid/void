...
export abstract class Drawable implements Box {
  ...
  get cel(): number {
    const iiic_cccc = this.#pool.view.getUint8(this.#offset + 9)
  ...
  get flipX(): boolean {
    const sxyz_llll = this.#pool.view.getUint8(this.#offset + 5)
  ...
  set flipY(flip: boolean) {
    ...
    this.#pool.view.setUint8(
      this.#offset + 5,
      (sxyz_llll & ~0x20) | (-flip & 0x20)
    )
  }

  get h(): number {
    const h12_wwww = this.#pool.view.getUint16(this.#offset + 7, true)
    return h12_wwww >>> 4
  }
  ...
  get id(): number {
    const r_i10_c5 = this.#pool.view.getUint16(this.#offset + 9, true)
  ...
  get w(): number {
    const hhhh_w12 = this.#pool.view.getUint16(this.#offset + 6, true)
  ...
  get x(): number {
    const y12_x20 = this.#pool.view.getUint32(this.#offset + 0, true)
  ...
  get y(): number {
    const sxyz_llll_y20_xxxx = this.#pool.view.getUint32(this.#offset + 2, true)
  ...
  set z(z: number) {
    const sxyz_llll = this.#pool.view.getUint8(this.#offset + 5)
    this.#pool.view.setUint8(this.#offset + 5, (sxyz_llll & ~0xf) | (z & 0xf))
  }
